...
// blau, Mueller hellgrau (Baecker bleibt weiss). Die
...
//
// DREI REZEPTE SIND WIEDER RAUS (v313): Traeger, Schmied und Geologe. Sie
// stehen unten mit Begruendung als Kommentar, damit niemand sie
// versehentlich neu erfindet. Kurz: ein Fenster, das die ganze Figur
// trifft, macht aus drei Kleidungsstuecken eines - und Violett gibt es in
// dieser Welt nicht. Wo eine Figur nur zu dunkel ist, gehoert der Hebel
// an die Modelltextur (tools/modelltextur.py), nicht an die Farbe.
//
...
const BERUFE={
  ...
  // smith: ZURUECKGENOMMEN (v313). Die Nachzieherei ("Ziel eine Stufe
  // heller", "hell 0.35") hat den schwarzen Klotz nur in einen grauen
  // verwandelt: Anthrazit auf eine ohnehin dunkle Schieferblau-Textur
  // ergab eine Figur, die von Kopf bis Fuss dasselbe Grau trug - 80,0 %
  // groesstes Farbfeld, der hoechste Wert aller 35 Figuren. Der Schmied
  // traegt jetzt die Farben seines Modells; erkennbar ist er an der
  // dunklen Lederschuerze. Aufgehellt wird ueber tools/modelltextur.py.
  ...
  // carrier: ZURUECKGENOMMEN (v313). Zinnober-Orange mit dem Fenster
  // 18-52 Grad traf nicht die Tunika, sondern die GANZE Figur - Kittel,
  // Hose und Stiefel des Traegers liegen alle in diesem Braun-Olive-Band.
  // Uebrig blieb eine einfarbig terrakottafarbene Gestalt; gemessen mit
  // tools/figurenpruefung.py 72,9 % groesstes Farbfeld gegen 37,5 % im
  // Mittel aller Figuren, der zweithoechste Wert ueberhaupt. Der Traeger
  // traegt jetzt die Farben seines Modells; erkennbar ist er an der Ware,
  // die er vor der Brust haelt - ein staerkeres Zeichen als jede
  // Kennfarbe. Aufgehellt wird ueber tools/modelltextur.py.
  ...
  // geo: ZURUECKGENOMMEN (v313). Lavendel-Grau war kein Missgriff im
  // Fenster, sondern in der Farbe: Violett kommt in dieser Welt sonst
  // nirgends vor. Auf dem Blatt waren 51,4 % der Figur violett (b>g und
  // r>g), im Spiel stand ein malvenfarbener Mann in einer Landschaft aus
  // Erdtoenen. Der Geologe traegt jetzt die Farben seines Modells;
  // erkennbar ist er am breitkrempigen Hut und am Hammer - dieselbe
  // Begruendung wie beim Spaeher weiter unten. Aufgehellt wird ueber
  // tools/modelltextur.py.
