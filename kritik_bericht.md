...
**Behoben in v330 — und die vermutete Ursache war die falsche.** Die Expansionsgewichtung gab es längst (`edW` 0,6 in `aiFindSpot`) und der Vorstoß-Posten ab Minute 20 ebenfalls; beide waren nicht das Problem. Gemessen war die Wurzel ein Bewegungsfehler: `freiHalten()` nahm den laufenden Wegpunkt nur für `u.wp` (Träger und Arbeiter) vom Sperrkreis aus, nicht für `u._det` — die Route, über die Soldaten in ihren Posten marschieren. Wer auf eine Fahne, einen Baum oder einen Felsbrocken zulief, der auf seiner Route lag, wurde jeden Takt auf genau den Kreisradius zurückgeschoben und stand damit für immer. Über drei Saaten hingen so 10 bis 13 Soldaten fest, während nur 2 bis 3 je in einem Posten ankamen; ein hängender Marsch zählt zudem als „enroute" und sperrte seinen Zielposten für jeden weiteren Nachschub. Leere Posten verschieben die Grenze aber nicht (`recalcTerritory`: `besetztWar`), und die Regel gegen die Postenflut („keine neuen Posten, solange zwei leer stehen und keine Rekruten da sind") legte daraufhin auch den Bau still — deshalb die Friedhofsruhe. Nach der Reparatur kommen alle Soldaten an (15 / 13 / 17 statt 2 / 3 / 3 besetzt, null Hänger), und über 60 Spielminuten gibt es auf allen drei Saaten Grenzkontakt und Kampf statt auf zweien gar nichts.
