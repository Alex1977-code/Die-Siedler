...
Bei `ter_cliff_tall.png` steht ausdruecklich
...
Kachelsatz für das Wegenetz. Alle Kacheln sind orthografische Draufsichten mit Alpha - die Perspektive entsteht erst beim Drapieren über das Höhenmodell, deshalb hier ausdrücklich KEINE isometrische Ansicht.

## 10.1 Reihenfolge und Referenzbilder

**M1 zuerst generieren und final absegnen.** M1 ist der Farb- und Steinstil-Anker für alle weiteren Kacheln. Liegt M1 farblich daneben, zieht sich der Fehler durch den ganzen Satz - lieber drei Versuche investieren.

| ID | Kachel | Referenzbild |
|---|---|---|
| M1 | Gerade N-S (Master) | —, zuerst generieren und final absegnen |
| T2 | Segment 60 Grad | M1 |
| T3 | Kurve S nach O, 90 Grad | M1 |
| T4 | Bogen S nach NO, 60 Grad | M1 |
| T5 | T-Kreuzung | M1 |
| T6 | Vierwege-Kreuzung | T5 |
| T7 | Y-Gabelung 60 Grad | T5 |
| T8 | Sackgasse | M1 |
| T9 | Trampelpfad gerade | M1 |
| T10 | Uebergang Pfad zu Pflaster | T9 (untere Haelfte) und M1 (obere Haelfte) |
| T11 | Hangsegment gestuft | M1 |
| T12 | Treppe im Hang | T11 |
| T13 | Verwittert und ueberwachsen | M1 |
| T14 | Platzpflaster vollflaechig | M1 |
| T15 | Platzrand zu Gras | T14 |
| T16 | Sechswege-Nabe | T6 |
| T17 | Decal-Sheet Details | M1 |

**Nicht generieren:** Gerade O-W entsteht durch Drehen von M1 um 90 Grad. Aus T2 bekommt ihr durch Drehen alle drei Richtungen des Dreiecksgitters.

## 10.2 Angepasst an unser Gitter und unseren Stil

Zwei Abweichungen von der ursprünglichen Vorlage, die bewusst so sind:

- **60 Grad statt 45 Grad.** Ihr fahrt ein Dreiecksgitter (0/60/120 Grad). Die Vorlage enthielt 45-Grad-Kacheln (Diagonale, weiche Kurve, Y-Gabelung), die nie sauber an eure Kanten anschließen würden. T2, T4 und T7 sind entsprechend umgestellt. Falls ihr doch auf Quadgitter wechselt: in den Prompts "60 degrees" durch "45 degrees" ersetzen und die Kantenpositionen auf Ecken statt 25-Prozent-Marken ändern.
- **Weicher Cartoon-Stil statt Pixel-Look.** Der STYLE-Absatz schließt Pixelation, Dithering und Retro-Palette ausdrücklich aus. Das Downsampling auf 128 Pixel mit anschließendem Nearest-Neighbor-Hochskalieren aus der Vorlage bitte NICHT anwenden - es erzeugt genau die harten Kanten, die wir überall sonst vermeiden.

**Warum nicht gruppiert:** Bei Wegekacheln muss die Bandposition und -breite an der Kachelkante exakt stimmen, sonst versetzen sich die Wege beim Aneinanderlegen. In einem Sammelbild kontrolliert das Modell die Panel-Ränder nicht präzise genug. Einzige Ausnahme ist T17, das von Natur aus ein Sheet ist.

**Konstant halten:** Die Absätze PALETTE, EDGES, LIGHTING, STYLE und FORMAT sind in allen Kacheln absichtlich wörtlich identisch. Wer sie umformuliert, bekommt Farbdrift zwischen den Kacheln.

## 10.3 Einbindung ins Höhenmodell

- **Kein Licht in der Textur** - daher der ausdrückliche Ausschluss von Schatten und gerichtetem Licht. Die Höhenwirkung kommt aus der Normal Map.
- **Normal Map aus dem Luminanzkanal ableiten**, nicht vom Bildmodell generieren lassen. Die Fugen liefern die Tiefeninformation bereits (dunkel = tief); bei Graustufen-Heightmaps produziert das Bildmodell unbrauchbare Werte.
- **Alpha-Rand großzügig lassen**, damit der Weg beim Drapieren auf Hänge nicht hart abbricht.
- **Auflösung:** 512 Pixel pro Gitterzelle ist reichlich, 256 reicht auch.

## 10.4 Die Prompts


**M1 - Gerade N-S (Master)** (ohne Referenzbild)
```
Top-down orthographic texture tile of a medieval cobblestone road for
a stylized cartoon settlement-building game.

ROAD: straight band running vertically through the exact center of the
tile, 35% of tile width. Densely packed irregular rounded fieldstones,
each stone about 1/40 of the tile width, varied size and rotation. Two
faint smoother ruts along the center from cart wheels.

PALETTE: stones #a49b8a #8d8676 #bdb4a0 #726c5e, mortar joints
#57503f, sand fringe #a8926e, dry grass #7d7a55.

EDGES: frayed and organic, never a straight cut. A 5% transition zone
of loose sand and gravel fading into full alpha transparency. A few
single displaced stones outside the main band, sparse tufts of dry
grass overlapping the edge.

LIGHTING: flat and even, no cast shadows, no directional light, no
baked ambient occlusion. Only subtle micro-shading inside the mortar
joints.

STYLE: soft painterly cartoon texture, chunky simplified shapes with
rounded soft edges, LOW contrast, muted desaturated palette, matte. No
black outlines, no pixelation, no dithering, no visible pixel grid, no
retro palette limitation, no gloss, no photorealism, no PBR
highlights.

TILING: the band exits the top and bottom edge at identical position
and width, seamless. Fully transparent background outside the road and
its sand fringe.

FORMAT: 512x512, square, no text, no border, no frame, no perspective,
no vignette, no drop shadow.
```


**T2 - Segment 60 Grad** (Referenz: M1)
```
REFERENCE: use the attached image M1. Match its exact stone shapes,
size distribution, palette, joint darkness, grain and edge fraying.
Same texture, different topology only.

Top-down orthographic texture tile of a medieval cobblestone road for
a stylized cartoon settlement-building game.

ROAD: straight band crossing the tile at a constant 60 degree angle,
entering the bottom edge at 25% of the tile width from the left and
exiting the right edge at 25% of the tile height from the top, 35% of
tile width measured perpendicular to the band. Stones oriented along
the band direction. Two faint smoother cart ruts.
...
TILING: the band exits the bottom and right edge at exactly those
positions with identical width and a constant 60 degree angle,
seamless. Fully transparent background outside the road and its sand
fringe.
...
**T3 - Kurve S nach O, 90 Grad** (Referenz: M1)
```
REFERENCE: use the attached image M1. Match its exact stone shapes,
size distribution, palette, joint darkness, grain and edge fraying.
Same texture, different topology only.
...
ROAD: a smooth 90 degree quarter-circle curve entering the bottom edge
at its center and exiting the right edge at its center, constant band
width of 35% of tile width. Stones re-oriented radially, following the
arc. Slightly denser and more worn on the inner side of the curve.
...
TILING: the band meets the bottom and right edge perpendicularly, at
the exact edge center and identical width, seamless. Fully transparent
background outside the road and its sand fringe.
...
**T4 - Bogen S nach NO, 60 Grad** (Referenz: M1)
```
REFERENCE: use the attached image M1. Match its exact stone shapes,
size distribution, palette, joint darkness, grain and edge fraying.
Same texture, different topology only.
...
ROAD: a gentle 60 degree bend entering the bottom edge at its center
pointing straight up, curving softly and exiting the right edge at 25%
of the tile height from the top. Constant band width of 35% of tile
width, wide sweeping radius, stones rotating gradually along the path.
...
TILING: the band meets the bottom edge perpendicularly at its exact
center and exits the right edge at 25% height with a constant 60
degree angle, identical width at both ends, seamless. Fully
transparent background outside the road and its sand fringe.
...
ROAD: a T-junction. One continuous horizontal band across the tile
from the left edge center to the right edge center, plus one branch
running down from the junction to the bottom edge center. All bands
35% of tile width. The junction area is slightly wider, more irregular
and visibly more worn, stones fanning out where the paths merge, ruts
...
TILING: bands exit the left, right and bottom edge at the exact edge
center with identical width, perpendicular to the edge, seamless. The
top edge stays fully transparent. Fully transparent background outside
the road and its sand fringe.
...
**T6 - Vierwege-Kreuzung** (Referenz: T5)
```
REFERENCE: use the attached image T5. Match its exact stone shapes,
size distribution, palette, joint darkness, junction wear and edge
fraying. Same texture, different topology only.

Top-down orthographic texture tile of a medieval cobblestone road for
a stylized cartoon settlement-building game.

ROAD: a symmetrical four-way crossing. Bands run from all four edge
centers into the middle of the tile, each 35% of tile width. The
central crossing area is the most worn part: densest, most irregular
stone placement, smoothed and lightened stones, cart ruts crossing in
both directions, a few sunken stones.
...
TILING: bands exit all four edges at the exact edge center with
identical width, perpendicular to the edge, seamless. Fully
transparent background outside the road and its sand fringe.
...
**T7 - Y-Gabelung 60 Grad** (Referenz: T5)
```
REFERENCE: use the attached image T5. Match its exact stone shapes,
size distribution, palette, joint darkness, junction wear and edge
fraying. Same texture, different topology only.
...
ROAD: a Y-shaped fork. One band enters at the bottom edge center, runs
up to the middle of the tile and splits into two branches at 60
degrees to each other, one exiting the left edge at 25% of the tile
height from the top, the other exiting the right edge at 25% of the
tile height from the top. All bands 35% of tile width. The wedge
between the two branches is a rounded triangular island of packed sand
and a few loose stones, not a sharp point.
...
TILING: the band meets the bottom edge perpendicularly at its exact
center, the two branches exit the left and right edge at 25% height
with identical width, seamless. Fully transparent background outside
the road and its sand fringe.
...
ROAD: a dead end. A vertical band enters at the bottom edge center,
35% of tile width, and terminates at about 60% tile height. The end is
ragged and unfinished: the last stones become sparse, half-buried and
irregular, dissolving into loose gravel and packed sand that fades to
transparency. No kerb, no rounded cap, no deliberate finish.
...
TILING: the band meets the bottom edge perpendicularly at its exact
center with the standard width, seamless. Top, left and right edges
stay fully transparent. Fully transparent background outside the road
and its sand fringe.
...
**T9 - Trampelpfad gerade** (Referenz: M1)
```
REFERENCE: use the attached image M1. Match its edge fraying, grain,
grass tufts and overall painting technique. The surface material is
different: compacted dirt instead of cobblestone.

Top-down orthographic texture tile of a medieval dirt footpath for a
stylized cartoon settlement-building game.

PATH: straight band running vertically through the exact center of the
tile, 30% of tile width. Compacted bare earth, faint footprints, small
embedded pebbles, a few shallow cart ruts, patches of dust.

PALETTE: dirt #6e5a41 #8a7355 #5b4a35 #9c8362, pebbles #a49b8a, sand
fringe #a8926e, dry grass #7d7a55.

EDGES: frayed and organic, never a straight cut. Grass creeping into
the path, a 5% transition zone fading into full alpha transparency,
sparse tufts of dry grass overlapping the edge.

LIGHTING: flat and even, no cast shadows, no directional light, no
baked ambient occlusion. Only very subtle micro-shading in the ruts.
...
TILING: the band exits the top and bottom edge at identical position
and width, seamless. Fully transparent background outside the path and
its fringe.
...
**T10 - Uebergang Pfad zu Pflaster** (Referenz: T9 (untere Haelfte) und M1 (obere Haelfte))
```
REFERENCE: use the attached image T9 for the lower half and the
attached image M1 for the upper half. Match both source textures
exactly and blend them.

Top-down orthographic texture tile showing a medieval dirt path
turning into a cobblestone road, for a stylized cartoon settlement-
building game.

LAYOUT: one continuous vertical band through the exact center of the
tile. The bottom third is compacted dirt path 30% of tile width. The
top third is fully cobbled road 35% of tile width. The middle third is
the transition: the band widens gradually, first stones appear
scattered and half-buried in the dirt, then become denser and finally
fully packed. No hard line, no visible seam between the two materials.

PALETTE: dirt #6e5a41 #8a7355 #5b4a35, stones #a49b8a #8d8676 #bdb4a0
#726c5e, mortar joints #57503f, sand fringe #a8926e, dry grass
#7d7a55.

EDGES: frayed and organic, never a straight cut. A 5% transition zone
fading into full alpha transparency, sparse tufts of dry grass
...
TILING: the dirt band exits the bottom edge center at 30% width, the
cobbled band exits the top edge center at 35% width, both
perpendicular, seamless. Fully transparent background outside the road
and its fringe.
...
**T11 - Hangsegment gestuft** (Referenz: M1)
```
REFERENCE: use the attached image M1. Match its exact stone shapes,
size distribution, palette, joint darkness, grain and edge fraying.
Same texture, different stone arrangement only.

Top-down orthographic texture tile of a medieval cobblestone road
built on a slope, for a stylized cartoon settlement-building game.

ROAD: straight band running vertically through the exact center of the
tile, 35% of tile width. The stones are arranged in horizontal courses
across the band, like shallow terraces holding the paving in place on
an incline. Larger flatter anchor stones every few courses. Loose
gravel collected against the downhill side of each course. Slightly
eroded, some gaps where stones have slipped.
...
LIGHTING: flat and even, no cast shadows, no directional light, no
baked ambient occlusion. Only subtle micro-shading inside the mortar
joints. Do not paint any slope shading or gradient, the incline comes
from geometry.
...
TILING: the band exits the top and bottom edge at identical position
and width, the stone courses continue across both edges, seamless.
Fully transparent background outside the road and its sand fringe.
...
**T12 - Treppe im Hang** (Referenz: T11)
```
REFERENCE: use the attached image T11. Match its exact stone shapes,
palette, joint darkness, course arrangement, grain and edge fraying.
Same texture, different topology only.

Top-down orthographic texture tile of a medieval stone stairway on a
steep slope, for a stylized cartoon settlement-building game.

STAIRS: straight flight running vertically through the exact center of
the tile, 35% of tile width, eight steps evenly spaced. Each step is a
row of long flat riser stones spanning the width, with smaller packed
cobbles filling the tread between them. Steps slightly irregular in
length and alignment, worn and rounded in the middle where people
walk, edges chipped.

PALETTE: stones #a49b8a #8d8676 #bdb4a0 #726c5e, mortar joints
#57503f, sand fringe #a8926e, dry grass #7d7a55.

EDGES: frayed and organic, never a straight cut. Loose gravel and
packed sand around the flight fading into full alpha transparency,
grass growing in the gaps at the sides.

LIGHTING: flat and even, no cast shadows, no directional light, no
baked ambient occlusion. Only subtle micro-shading in the joints
between riser and tread. Do not paint step shadows, the height comes
from geometry.
...
TILING: the flight exits the top and bottom edge at identical position
and width, the step rhythm continues across both edges, seamless.
Fully transparent background outside the stairway and its fringe.
...
**T13 - Verwittert und ueberwachsen** (Referenz: M1)
```
REFERENCE: use the attached image M1. Match its exact stone shapes,
size distribution, palette, joint darkness and grain. Same texture,
aged and overgrown variant.

Top-down orthographic texture tile of an old neglected medieval
cobblestone road, for a stylized cartoon settlement-building game.

ROAD: straight band running vertically through the exact center of the
tile, 35% of tile width. Same paving as the reference but decayed:
moss and weeds growing thickly in the joints, several stones missing
leaving shallow dirt patches, other stones tilted and sunken, a puddle
of packed mud in one dip. Grass has reclaimed roughly a quarter of the
surface, mostly along the edges and in irregular patches toward the
center.

PALETTE: stones #a49b8a #8d8676 #bdb4a0 #726c5e, mortar joints
#57503f, moss #5f6b40 #47522f, mud #6e5a41, sand fringe #a8926e, dry
grass #7d7a55.

EDGES: heavily frayed, grass overgrowing far into the band, an 8%
transition zone fading into full alpha transparency, many displaced
stones scattered outside.

LIGHTING: flat and even, no cast shadows, no directional light, no
baked ambient occlusion. Only subtle micro-shading inside the joints
and moss.
...
**T14 - Platzpflaster vollflaechig** (Referenz: M1)
```
REFERENCE: use the attached image M1. Match its exact stone shapes,
size distribution, palette, joint darkness and grain. Same texture,
filling the whole tile.

Top-down orthographic texture tile of a fully paved medieval market
square, for a stylized cartoon settlement-building game.

SURFACE: the entire tile is covered edge to edge with the same
cobblestone paving as the reference, no road band, no fringe, no
transparency. Stones laid in loose irregular fan patterns typical of
hand-set cobbles. Subtle variation in stone density and wear across
the tile, a few slightly sunken areas, a scattering of sand in the
joints. No focal point, no center motif, no drain, no pattern that
would read as a repeating logo when tiled.

PALETTE: stones #a49b8a #8d8676 #bdb4a0 #726c5e, mortar joints
#57503f, sand in joints #a8926e.
...
TILING: fully seamless on all four edges, stones continue across every
border without a visible seam, no border stones, no frame of any kind.
Opaque, no alpha.
...
**T15 - Platzrand zu Gras** (Referenz: T14)
```
REFERENCE: use the attached image T14. Match its exact stone shapes,
size distribution, palette, joint darkness and grain. Same paving, now
ending in a ragged edge.

Top-down orthographic texture tile showing the outer edge of a paved
medieval square meeting bare ground, for a stylized cartoon
settlement-building game.

LAYOUT: the top 55% of the tile is fully paved cobblestone identical
to the reference. Toward the bottom the paving breaks up: stones
become sparse, half-buried and displaced, sand and gravel fill the
gaps, and by the bottom 20% the surface is fully transparent. The
break line is irregular and wandering across the tile, never straight,
with a few isolated stones stranded far out. Tufts of dry grass push
up through the outermost joints.
...
TILING: the top edge and both side edges are fully paved and seamless
with the reference tile. The bottom edge is fully transparent.
...
**T16 - Sechswege-Nabe** (Referenz: T6)
```
REFERENCE: use the attached image T6. Match its exact stone shapes,
size distribution, palette, joint darkness, junction wear and edge
fraying. Same texture, different topology only.

Top-down orthographic texture tile of a medieval cobblestone road hub
with six converging paths, for a stylized cartoon settlement-building
game.

ROAD: six bands radiating from the exact center of the tile at 60
degree intervals, each 30% of tile width, arranged in perfect radial
symmetry with one band pointing straight up. The center is a rounded
worn plaza where all bands merge, stones laid in a loose circular fan,
heavily smoothed and lightened by traffic, ruts running in from every
direction.
...
EDGES: frayed and organic, never a straight cut. A 5% transition zone
of loose sand and gravel fading into full alpha transparency, single
displaced stones scattered between the arms, tufts of dry grass in the
six wedges.
...
TILING: all six bands reach the tile border at identical width and
fade out there. Fully transparent background outside the road and its
sand fringe.
...
**T17 - Decal-Sheet Details** (Referenz: M1)
```
REFERENCE: use the attached image M1. Match its exact stone shapes,
size distribution, palette, joint darkness and grain.

Top-down orthographic sprite sheet of small medieval road detail
decals for a stylized cartoon settlement-building game.

LAYOUT: a 4 by 4 grid of sixteen separate small elements, evenly
spaced, each element clearly isolated with transparent space around
it, no grid lines, no labels, no numbers. Contents: single loose
cobblestones in four sizes, a small pile of gravel, a patch of packed
sand, a shallow mud puddle, a pair of cart rut segments, a scatter of
pebbles, a broken half-sunken stone, a tuft of dry grass, a moss
patch, a small dirt scuff, a wheel track curve, a group of three
stones, a worn hollow in the paving.

PALETTE: stones #a49b8a #8d8676 #bdb4a0 #726c5e, mortar joints
#57503f, mud #6e5a41, sand #a8926e, moss #5f6b40, dry grass #7d7a55.

LIGHTING: flat and even, no cast shadows, no directional light, no
baked ambient occlusion, no drop shadows under the loose stones.
...
FORMAT: 512x512, square, fully transparent background, no text, no
border, no frame, no perspective, no vignette.
```

---

# 11. Grafikauftrag v62 (Bedienung, Muehle, Acker, Baustellen, Wegenabe)

Alle Prompts vollstaendig ausgeschrieben. Zwei Abweichungen vom Auftragstext, beide bewusst:

- **Die drei Ackerbilder brauchen ein Referenzbild.** "Identische Furchen-Ausrichtung" reicht fuer das Ueberblenden nicht - Furchenabstand UND Furchenposition muessen exakt uebereinstimmen, sonst wandern die Reihen beim Ueberblenden seitlich. Deshalb `ter_field_0` zuerst, die anderen beiden mit ihm als Referenz.
- **Die Baustellen als Sheet statt vier Einzelbilder.** Vier separat generierte Bilder driften in Grundflaeche und Groesse auseinander. Im Sheet stehen alle vier Stufen auf derselben Grundflaeche und im selben Massstab - dasselbe Verfahren wie bei den drei Groessenklassen in Kapitel 2.5. Aus dem Sheet schneidet ihr die vier Dateien.

Ebenfalls per Referenzbild: `ui_cancel` nutzt `ui_ok`, damit Knopfform, Durchmesser, Goldrand und Woelbung exakt gleich sind. `road_hub` nutzt `road_str`.

## 11.1 Bedienung

**Bestaetigungs-Knopf** - `ui_ok.png` (256x256)
```
A stylized cartoon UI button for a medieval settlement-building game,
matching a warm hand-painted earthy art style: a round button of dark
stained oak with a raised gold rim running all the way around it. On
its face, one bold thick check mark in warm ivory-gold, simple and
clean with rounded ends, filling most of the button face.

The button is gently domed so it reads as a physical object, with a
narrow darker shadow edge along its lower rim so it appears to stand
slightly proud of the surface behind it.

Chunky simplified shapes, bold and readable: in the game this button
is drawn only about 48 pixels across, so no thin lines, no fine
detail, no small ornament. The button fills roughly 80 percent of the
image width, centered, with even empty margin all around.

Seen straight on from the front, flat to the camera, no perspective
distortion, no tilt. Soft light from the upper left. Plain solid white
background. No text, no lettering, no numbers, no watermark, no UI
frame around it, no cast shadow on the background.
```

**Abbrechen-Knopf** - `ui_cancel.png` (256x256, Referenz: `ui_ok.png`)
```
REFERENCE: use the attached image ui_ok. Match its exact button shape,
diameter, wood tone, gold rim, doming, shadow edge and lighting
precisely. The only difference is the symbol on the face.

A stylized cartoon UI button for a medieval settlement-building game:
a round button of dark stained oak with a raised gold rim. On its
face, one bold thick cross in muted deep red, two simple strokes with
rounded ends crossing at the center, filling most of the button face.
...
**Namensschild** - `ui_schild.png` (512x192)
```
A stylized cartoon UI name plate for a medieval settlement-building
game, matching a warm hand-painted earthy art style: a narrow
horizontal board of dark stained oak with a thin gold border line
running around its edge, the two short ends softly rounded.

CRITICAL - the plate is stretched horizontally in the game: the entire
middle section must be completely EMPTY and evenly toned, a plain flat
wood surface with no grain feature, no ornament, no nail, no highlight
and no tonal variation that would smear when stretched. All decoration
sits exclusively at the two short ends: a small carved corner flourish
and one iron rivet at each end, mirrored left and right.

Chunky simplified shapes, bold and readable: in the game this plate is
drawn only about 20 pixels tall, so no thin lines and no fine
ornament. The plate spans nearly the full image width with a small
even margin.

Seen straight on from the front, flat to the camera, no perspective
distortion, no tilt. Soft light from the upper left. Plain solid white
background. No text, no lettering, no numbers, no watermark, no cast
shadow on the background.
```

**Meldungstafel** - `ui_toast.png` (768x224)
```
A stylized cartoon UI message banner for a medieval settlement-
building game, matching a warm hand-painted earthy art style: a wide
horizontal panel of aged parchment mounted on a dark stained oak
backing, with a thin gold border line running around its edge and a
small carved scroll ornament in each of the four corners.

CRITICAL - the banner is stretched horizontally in the game: the
entire middle section must be completely EMPTY and evenly toned, a
plain flat parchment surface with no stain, no crease, no ornament and
no tonal variation that would smear when stretched. All decoration
sits exclusively at the two short ends and the four corners.

Chunky simplified shapes, bold and readable at small size: no thin
lines, no fine ornament, no filigree. The banner spans nearly the full
image width with a small even margin.
...
**Fluegelkreuz** - `obj_millsails.png` (512x512) - UEBERHOLT, siehe `muehlenrad_ortho.png` in 12.6

*Diese Fassung hat die Schraegsicht ins Bild eingebacken. Fuer ein im Code gedrehtes Sprite ist das falsch: die Verkuerzung dreht sich mit und das Rad wirkt taumelnd statt drehend. Richtig ist das flache, rotationssymmetrische `muehlenrad_ortho.png`, das erst gedreht und danach gestaucht wird.*
```
A stylized cartoon windmill sail cross for a medieval settlement-
building game, matching a warm hand-painted earthy art style. The sail
cross ONLY - no tower, no building, no roof, no landscape.

OBJECT: four wooden sail blades radiating outward at exactly 90
degrees from a central hub, each blade a slim wooden lattice frame
with pale cream sailcloth stretched across it. At the very center sits
a chunky round wooden hub with a short protruding iron-collared axle
stub pointing toward the viewer, so the cross clearly looks like it
mounts ONTO something rather than floating in front of it.

PERSPECTIVE - important: the cross is tilted back slightly, seen from
a mildly elevated angle like the game's buildings, NOT flat frontal.
The upper pair of blades is therefore slightly shorter and smaller
than the lower pair, giving the whole cross a sense of depth.

CRITICAL for animation: the central hub must sit at the EXACT center
of the image and the four blades must be evenly balanced around it, so
the whole cross can be rotated around that center point. Equal empty
margin on all four sides, generous enough that the blade tips stay
inside the frame at every rotation angle.

Soft light from the upper left, with gentle self-shadowing where a
blade frame casts a soft shadow onto its own sailcloth. Chunky
simplified shapes, minimal fine detail.

Plain solid white background. No text, no watermark, no UI, no tower,
no building, no sky, no ground, no cast shadow on the background.
```

## 11.3 Acker

**Frisch gepfluegt** - `ter_field_0.png` (512x512)
```
A seamless top-down GROUND texture of freshly ploughed farmland for a
stylized cartoon medieval settlement-building game.

SURFACE: bare turned brown earth laid out in straight HORIZONTAL
furrow rows running across the full width of the tile, evenly spaced,
with soft rounded ridges between them. A scattering of small clods and
crumbs of soil across the surface. Warm muted brown palette with
gentle tonal variation between rows.

This texture is laid over a 3D terrain mesh, so it must carry NO
lighting information of its own: completely flat, even, ambient
illumination, no directional light, no cast shadows, no ambient
occlusion, no baked highlights. All shading comes from the mesh.

Soft painterly cartoon look, chunky simplified shapes, LOW contrast,
no black outlines, no pixelation, no fine noise.

Seen from directly above, flat orthographic top-down view, perfectly
square, seamlessly tileable on all four edges with no visible seam,
the furrow rows continuing across the left and right edges. Filling
the entire frame edge to edge. No border, no field frame, no fence, no
path, no text, no watermark, no objects, no characters, no vignette.
```

**Junger Aufwuchs** - `ter_field_1.png` (512x512, Referenz: `ter_field_0.png`)
```
REFERENCE: use the attached image ter_field_0. Match its furrow
spacing, furrow positions, row phase, soil color and painting style
EXACTLY - the rows must line up perfectly with the reference so the
game can cross-fade between the two textures without the rows
shifting. Only the crop growing in the rows is different.

A seamless top-down GROUND texture of young growing crops for a
stylized cartoon medieval settlement-building game.

SURFACE: the same ploughed brown earth in the same straight HORIZONTAL
furrow rows, now with fresh green cereal shoots sprouting along each
row - short, sparse and upright, so the bare soil between the rows
remains clearly visible. Muted fresh green against warm brown.
...
Seen from directly above, flat orthographic top-down view, perfectly
square, seamlessly tileable on all four edges with no visible seam,
the rows continuing across the left and right edges. Filling the
entire frame edge to edge. No border, no field frame, no fence, no
path, no text, no watermark, no objects, no characters, no vignette.
```

**Erntereif** - `ter_field_2.png` (512x512, Referenz: `ter_field_0.png`)
```
REFERENCE: use the attached image ter_field_0. Match its furrow
spacing, furrow positions, row phase and painting style EXACTLY - the
rows must line up perfectly with the reference so the game can cross-
fade between the textures without the rows shifting. Only the crop
growing in the rows is different.

A seamless top-down GROUND texture of ripe grain ready for harvest,
for a stylized cartoon medieval settlement-building game.

SURFACE: dense golden-yellow cereal grown up along the same straight
HORIZONTAL rows, tall and full, with visible seed heads, the stalks
leaning slightly. The crop now covers almost the entire surface - only
the narrowest hints of soil remain visible between the rows. Warm
muted golden palette.
...
**Turm-Baustelle (Sheet, ergibt _0 bis _3)** - `bld_build_turm_0.png bis _3.png` (je 512x512 nach dem Zuschnitt)
```
Stylized cartoon 3D game assets, chunky simplified shapes with rounded
edges and minimal fine surface detail, smooth matte surfaces, soft
volumetric shading with a clear light-to-shadow boundary, warm muted
earthy medieval color palette.
...
stages must occupy the SAME ground footprint and be drawn at the SAME
scale - the site does not grow wider, it only grows upward as it is
built. The footprint is ROUND and narrow, about half the width of a
one-storey cottage, but the structure grows tall. Each stage must be
clearly and obviously further along than the one before it.

Include exactly these four stages, left to right:

1. STAGE 0 - staked round plot: a cleared, levelled CIRCULAR patch of
bare brown earth ringed by thin wooden stakes with string strung
between them, a shallow dug ring trench along the outline, a shovel
stuck in a small dirt pile. Nothing built upward yet, completely flat.

2. STAGE 1 - round foundation and low frame: a low circular stone
foundation course now complete and level, with a short timber frame of
upright posts standing on it, no walls and no roof. Stacked timber
beams and a bucket of mortar on the ground beside it.

3. STAGE 2 - tall round scaffold: a tall circular timber scaffold of
lashed poles and planked working platforms rising well above the
foundation, with a ladder leaning against it, but almost no masonry
built up inside it yet. Stacked stone blocks waiting at the base.

4. STAGE 3 - scaffolded tower shell: a tall round stone tower shell
now built up inside the scaffolding, its walls raw and unfinished with
an open top and no roof, the scaffold and ladder still standing around
it, roofing timber waiting on the ground.
...
**Stollen-Baustelle (Sheet, ergibt _0 bis _3)** - `bld_build_mine_0.png bis _3.png` (je 512x512 nach dem Zuschnitt)
```
Stylized cartoon 3D game assets, chunky simplified shapes with rounded
edges and minimal fine surface detail, smooth matte surfaces, soft
volumetric shading with a clear light-to-shadow boundary, warm muted
...
Generate this as ONE single image containing four construction stages
of the SAME mine entrance site, arranged in a row from left to right
in chronological build order, with generous empty space between them
so each can be cropped out individually afterward. CRITICAL: all four
stages must occupy the SAME ground footprint and be drawn at the SAME
scale - the site does not grow wider as it progresses. The whole site
is small, about half the width of a one-storey cottage, and set
against a suggested low rocky slope at the back. Each stage must be
clearly and obviously further along than the one before it.

Include exactly these four stages, left to right:

1. STAGE 0 - marked adit mouth: a small patch of cleared ground at the
foot of a low rock face, the intended tunnel opening outlined on the
rock with thin wooden stakes and string, a pick and a shovel leaning
against the stone. No excavation yet.

2. STAGE 1 - first timbering: a shallow opening broken into the rock
with a rough wooden door frame of two uprights and a lintel set into
it, a heap of loose excavated rubble piled to one side.

3. STAGE 2 - finished portal: the timber portal complete and
reinforced with angled support beams and planking across the top, the
dark tunnel mouth clearly open behind it, stacked planks and a
wheelbarrow of rubble beside it.

4. STAGE 3 - miners hut shell: the finished portal plus the raw shell
of a small timber miners hut built beside it, walls up but roof not
yet covered, bare rafters showing, a short scaffold and a ladder
against one wall.
...
**Sechswege-Nabe** - `road_hub.png` (256x256, Referenz: `road_str.png`)
```
REFERENCE: use the attached image road_str. Match its exact stone
shapes, size distribution, palette, joint darkness, band width, grain
and edge fraying. Same texture, different topology only.

Top-down orthographic texture tile of a medieval cobblestone road hub
with six converging paths, for a stylized cartoon settlement-building
game.

ROAD: six short bands radiating from the exact center of the tile at
precisely 60 degree intervals - at 0, 60, 120, 180, 240 and 300
degrees - in perfect radial symmetry with one band pointing straight
up. Each band the same width as the straight road tile in the
reference. The center is a rounded worn plaza where all six bands
merge, stones laid in a loose circular fan, heavily smoothed and
lightened by traffic, ruts running in from every direction.
...
rounded soft edges, LOW contrast, muted desaturated palette, matte. No
black outlines, no pixelation, no dithering, no visible pixel grid, no
gloss, no photorealism, no PBR highlights.

TILING: all six bands reach the tile border at identical width and
fade out there. Fully transparent background outside the road and its
sand fringe.

FORMAT: 256x256, square, no text, no border, no frame, no perspective,
no vignette, no drop shadow.
```
...
Sammelt Auftrag 3, den Kritikbericht und die Gebirge-Ueberarbeitung. Gegenueber der Auftragsliste sind fuenf Positionen gruppiert, weil dort die Konsistenz zwischen den Einzelbildern entscheidend ist - aus 21 Dateien werden 13 Generierungslaeufe:

| Sheet | ergibt | warum gruppiert |
|---|---|---|
| Minen-Baustellen | `bld_build_mine_1` bis `_3` | gleiche Felsdom-Silhouette und Grundflaeche ueber alle drei Stufen |
| Fels-Objekte | `ter_scree_cone`, `obj_rockspire_1` bis `_3` | Facettengroesse, drei Tonwerte und Grauton muessen identisch sein |
| Gebaeude-Effekte | `fx_steam`, `fx_flour`, `fx_sawdust`, `fx_rings` | gleiche Weichheit und Vereinfachungsgrad |
| Werkzeugkoepfe | sieben `tool_*` | gleicher Massstab, gleicher Blickwinkel, gleicher Holz- und Metallton |
| Fahnenmenue-Knoepfe | drei `ui_btn_*` | identische Knopfform und Goldrand |

Nicht gruppierbar sind alle nahtlos kachelbaren Texturen - im Sammelbild geht die Kachelbarkeit verloren.

**Referenzbilder:** Minen-Baustellen und `bld_granitemine` nutzen `bld_coalmine`, `ter_field_1` nutzt `ter_field_0`, die Fahnenknoepfe nutzen `ui_ok`.

**Ersetzt aeltere Fassungen:** `ter_water` ersetzt die aeltere Wasserkachel, `ter_field_1` die erste Lieferung. Die frueheren Fassungen von `gebirge3`, `ter_ridge_snow`, `ter_scree_cone` und den drei `obj_rockspire` sind aus Kapitel 3.1 entfernt - gueltig ist allein die Stil-Master-Kette in 12.3.

## 12.1 Minen-Baustellen

**Sheet, drei Stufen** - `bld_build_mine_1.png bis _3.png` (je ca. 380x420 nach dem Zuschnitt, Referenz: `bld_coalmine.png`)
```
REFERENCE: use the attached image bld_coalmine. Match its CAMERA ANGLE
and VIEWING PERSPECTIVE exactly - a rounded rock dome seen from the
front with the tunnel entrance facing toward the viewer. Do NOT paint
a steep top-down view and do NOT show a ground diamond or floor plane
in front of the entrance. Also match its rock colour, moss tone,
painting style and light direction.

Stylized cartoon 3D game assets for a medieval settlement-building
game, chunky simplified shapes with rounded edges, minimal fine
surface detail, smooth matte surfaces, soft volumetric shading, warm
muted earthy palette.

Generate this as ONE single image containing three construction stages
of the SAME mine site, arranged in a row from left to right in
chronological build order, with generous empty space between them so
each can be cropped out individually afterward. CRITICAL: all three
stages share the SAME rock dome silhouette, the SAME ground footprint
and the SAME scale - the site does not grow wider, only the timbering
progresses. The rock dome fades out SOFTLY at its left and right sides
into nothing, with no hard vertical cut edge anywhere.

Include exactly these three stages, left to right:

1. STAGE 1 - a low rounded rock dome with a little moss on top, the
tunnel mouth just broken open at its front as a shallow dark recess,
with one single rough timber beam laid across the top of the opening.
Nothing else built.

2. STAGE 2 - the same rock dome, the tunnel mouth now framed by a
complete timber door frame of two uprights and a lintel, with a stack
of sawn planks leaning beside it and the first few sleepers and rails
of a mine cart track laid on the ground running out of the entrance.

3. STAGE 3 - the same rock dome, the portal now fully finished and
reinforced with angled support beams and planking across the top, a
small timber scaffold standing against one side of the dome. No mine
cart, no ore heap yet.

All three drawn at the same isometric 3/4 view with the same fixed
light direction from the upper left and a soft contact shadow beneath
each. All standing on the same ground line. Plain solid white
background throughout. No panel borders, no grid lines, no labels, no
text, no numbers, no UI, no characters, no background scenery, no
watermark.
```

## 12.2 Reparatur-Ersatz

**Granitmine** - `bld_granitemine.png` (320x300, Referenz: `bld_coalmine.png`)
```
REFERENCE: use the attached image bld_coalmine. Match its rock dome
silhouette, camera angle, rock colour, moss tone, painting style and
overall proportions. Only the working detail differs - this is a
granite quarry instead of a coal mine.

Stylized cartoon 3D game asset for a medieval settlement-building
game, chunky simplified shapes with rounded edges, minimal fine
surface detail, smooth matte surfaces, soft volumetric shading, warm
muted earthy palette.

OBJECT: a granite quarry cut into a rounded rocky hill - a small
timber hut with a shingled roof set against the rock face, a squared-
off quarry cut in the stone beside it showing clean chiselled steps,
and a few neatly stacked cut granite blocks on the ground in front. A
little moss on the upper rock.

CRITICAL - light direction: soft light from the UPPER LEFT,
brightening the left-facing surfaces and shading the right-facing
ones. Do not light this from the right.

Isometric 3/4 view at a fixed 40-degree elevated camera angle. The
rock fully covers the lower part of the composition with no gap
showing through, about 4 to 6 percent empty space above the highest
point, and the bottom edge of the object sits exactly on the ground
line at the bottom of the frame. Soft contact shadow beneath.

Plain solid white background. No text, no watermark, no UI, no
characters, no background scenery, no mine cart, no ore heap.
```

**Acker junger Aufwuchs** - `ter_field_1.png` (512x512, Referenz: `ter_field_0.png`)
```
REFERENCE: use the attached image ter_field_0. Match its furrow
spacing, furrow positions, row phase, soil colour and painting style
EXACTLY - the rows must line up perfectly with the reference so the
game can cross-fade between the textures without the rows shifting.

A seamless top-down GROUND texture of young growing grain for a
stylized cartoon medieval settlement-building game.

SURFACE: the same ploughed brown earth in the same straight HORIZONTAL
furrow rows, now with young green cereal shoots growing along every
single row. CRITICAL - the green must be UNMISTAKABLE: the shoots are
tall enough and dense enough that roughly half of the visible surface
reads as fresh green, forming continuous green ribbons along each row,
clearly distinguishable from bare ploughed soil even when the texture
is scaled down to a fifth of its size. Bare brown earth remains
visible only as narrow strips between the green rows.
...
## 12.3 Gebirge im Facetten-Stil (Stil-Master-Kette)

Drei Generierungen statt sechs. Ein Bild legt den Stil fest, alle weiteren erben ihn per Referenz-Upload; die vier Objekte entstehen gemeinsam auf einem Blatt, weil Bilder aus derselben Generierung nicht auseinanderdriften.

```
  1  gebirge3.png ................ Material-Master
       |                            (Facettengroesse, Kantensaum, 3 Tonwerte)
       +--> 2  ter_ridge_snow.png   (Referenz: 1, Palette ueberschrieben)
       |
       +--> 3  Objektblatt          (Referenz: 1) - ein Bild, vier Objekte
                 |
                 +- obj_rockspire_1.png
                 +- obj_rockspire_2.png
                 +- obj_rockspire_3.png
...
### Zwei Regeln fuer die Referenz-Durchlaeufe

**Stiltext streichen, sobald ein Referenzbild dabei ist.** Ausgeschriebene Palette- und Stilbeschreibungen konkurrieren mit dem Referenzbild und verwaessern die Uebernahme. Die Prompts 2 und 3 sind deshalb bewusst knapp - nur Form, Perspektive, Verbote.

**Transparenz-Modus schaltet Referenzbilder ab.** Das Objektblatt entsteht deshalb auf Weiss und wird nach dem Zerschneiden freigestellt. Bei den harten Facettensilhouetten ist das Keying unkritisch.

### 1. Material-Master

`gebirge3.png` - 1024x1024, nahtlos, vollflaechig, OHNE Referenzbild.

Dieses Bild traegt alles Weitere, also ruhig drei bis vier Durchlaeufe investieren. Schwierigste Vorgabe ist die fehlende Lichtrichtung. Kontrolle: Bild in Graustufen ansehen - ist eine Ecke sichtbar heller als die gegenueberliegende, verwerfen.

```
Seamless tileable rock surface texture, orthographic top-down view,
looking straight down, completely flat lighting.

Surface built from large flat polygonal stone plates, each plate
roughly 1/5 to 1/6 of the image width, irregular angular shapes with
crisp straight edges, packed edge to edge like dry-stone pavement.
Plates separated by narrow dark joints. Each plate carries an even,
slightly brighter rim along all of its edges; the plate interior is a
single flat tone with only very subtle mottling.

Exactly three tonal values, distributed randomly across the plates:
light #C9C0B0, mid #9A9080, dark #6A6255. Warm light grey to beige
stone.

CRITICAL: no light direction whatsoever. No directional light, no cast
shadows, no ambient occlusion, no bevel, no gradient across the image,
no bright corner, no dark corner, no vignette. The image must be
uniformly lit from edge to edge so that external shading can be
applied later.

No grit, no sand, no gravel, no speckles, no fine-grained noise, no
crack network, no moss, no lichen, no vegetation, no snow, no water.

Fills the entire frame edge to edge and is seamlessly tileable on all
four sides: the pattern at the left edge continues into the right
edge, the top edge into the bottom edge. No border, no frame, no drop
shadow. Soft cartoon game art, hand-painted, no outlines, no text, no
watermark.
```

### 2. Firn

`ter_ridge_snow.png` - 1024x1024, danach auf 512x512. **Referenzbild: `gebirge3.png`.** Die Palette wird bewusst ueberschrieben, die Facettensprache bleibt.

```
Match the facet language of the uploaded image: same plate size, same
crisp straight edges, same brighter edge rim, same flat three-tone
treatment, same hand-painted cartoon finish. Keep those rules, replace
the material with snow.

Seamless tileable snow surface, orthographic top-down view, looking
straight down. Wind-packed alpine firn on a high ridge.

Parallel wind ripples (sastrugi) running diagonally across the
surface, carved into hard angular forms: each drift ends in a sharp
broken edge, a small vertical step, never a soft rounded dune. Flat
facet tops between the ripples.

Colours override the reference: bright snow #F2F4F6, mid #D6DDE4, cool
blue-grey shadow #A9B6C2. No warm grey, no beige, no saturated blue.

Sparingly scattered: four or five small angular dark rock windows
breaking through the snow, warm grey #6A6255, none touching the image
border.

Flat even lighting, no light direction, no cast shadows, no gradient
across the image, no bright or dark corner, no vignette. No sparkle,
no glitter, no ice crystals, no footprints, no tracks, no fine-grained
noise, no vegetation.

Fills the frame edge to edge, seamlessly tileable on all four sides.
No border, no frame, no text.
```

### 3. Objektblatt

Ein Bild, vier Objekte. **Referenzbild: `gebirge3.png`.** Weisser Hintergrund, kein Transparenz-Modus.

Der Absatz zur Facettengroesse ist der eigentliche Klebstoff - er haelt die Facetten ueber alle vier Objekte gleich gross, dadurch wirken sie wie aus einem Steinbruch. Der Groessenanker verhindert, dass alle vier gleich gross gerendert werden.

```
Match the rock material of the uploaded image exactly: same facet
size, same crisp straight edges, same brighter edge rim, same three
flat tonal values, same warm grey-beige stone, same hand-painted
cartoon finish.

A sheet of four separate rock formations on a plain flat white
background, arranged in a clean 2x2 grid, one formation per quadrant,
wide empty white margins between them, no dividing lines, no labels,
no shadows on the background.

Isometric three-quarter view, camera 40 degrees above the horizon,
identical camera for all four. Light from the upper LEFT for all four:
facets facing up and left take the bright tone, facets facing right
and down take the dark tone. Hard tonal breaks along facet edges, no
gradients, no soft shading, no rim light.

TOP LEFT - a slender stepped spire: tall and narrow, built as a stack
of flat-topped rock steps of decreasing size, each step offset from
the one below, silhouette clearly stepped and terraced, never a smooth
cone. A few angular rubble blocks at its foot.

TOP RIGHT - a terraced block tower: bulky, four or five massive
stacked cuboid blocks of different sizes, each rotated slightly
against the one below, forming broad flat terraces and ledges. One
block juts out as a clear overhang. Flat plateau on top. Angular
rubble blocks at its foot.

BOTTOM LEFT - a squat boulder cluster: wider than tall, five or six
massive angular blocks of clearly different sizes leaning against each
other, one dominant block off centre, smaller sharp wedges filling the
gaps. No rounded boulders.

BOTTOM RIGHT - a talus scree fan: narrow apex at the top of its
quadrant, spreading downward and outward to a wide base. Built
entirely from angular faceted chips, wedges and slabs. Coarse large
blocks near the apex, getting progressively smaller and denser toward
the outer edge. Jagged outline made of individual block silhouettes,
never a smooth curve.

Relative size, this matters: the terraced block tower is the baseline
at 1 unit tall. The slender spire is 1.3 units tall and 0.35 units
wide. The boulder cluster is 0.55 units tall and 1.1 units wide. The
scree fan is 0.5 units tall and 1.6 units wide.

The individual rock facets are the SAME absolute size on all four
formations, so the four objects read as the same stone at the same
scale.

No ground shadows, no cast shadows on the background, no ground plane,
no base plates, no grass, no snow, no moss, no vegetation, no clouds,
no birds, no fine-grained noise, no outlines, no text, no watermark.
Each formation stays well inside its own quadrant with a generous
margin and never touches the frame edge or the centre lines.
```

**Zerlegen:**

1. Blatt in vier 512x512-Quadranten schneiden.
2. Jedes Objekt in seinem Quadranten neu mittig ausrichten - das Modell trifft die Quadrantenmitte selten exakt.
3. Freistellen gegen Weiss.
4. Scree-Fan-Quadrant auf 512x384 beschneiden: oben 128 px weg, das Objekt sitzt in der unteren Haelfte.
5. Speichern als `obj_rockspire_1.png`, `_2`, `_3`, `ter_scree_cone.png`.

### 4. Optional: Einzeldurchlaeufe fuer mehr Detail

Nach dem Schnitt hat jedes Objekt nur rund 450 px Nutzflaeche. Reicht das fuer 512x512-Assets nicht, jedes Objekt noch einmal einzeln auf 1024x1024 generieren - **mit dem Objektblatt als Referenzbild.** Fuer die anderen drei nur die Quadrantenangabe tauschen: `TOP RIGHT`, `BOTTOM LEFT`, `BOTTOM RIGHT`.

```
Match the uploaded sheet exactly: same rock material, same facet size,
same three flat tonal values, same isometric 40 degree camera, same
light from the upper left, same hand-painted cartoon finish.

Render ONLY the formation from the TOP LEFT quadrant of the sheet,
alone, centred, large in frame, on a plain flat white background.

No ground shadow, no cast shadow, no ground plane, no base plate, no
vegetation, no snow, no outlines, no text. Leave a small margin all
around; the object must not touch the frame edge.
```

### Prueflliste vor dem Einbau

| Pruefung | Bilder | Kriterium |
|---|---|---|
| Graustufen-Test | Master, Firn | keine Ecke heller als die gegenueberliegende |
| Versatz-Test 50/50 | Master, Firn | Kreuznaht unsichtbar, sonst retuschieren |
| Tonwert-Zaehlung | alle | Histogramm zeigt drei Haeufungen, keinen Verlauf |
| Facettenmass | Objekte | Facetten auf allen vier Objekten gleich gross |
| 20-Prozent-Verkleinerung | alle | Facetten bleiben als Flaechen lesbar |
| Alphakante | Objekte | kein weisser Saum, kein Bodenschatten mitgekeyt |
| Lichtrichtung | Objekte | helle Flaechen oben links, dunkle unten rechts |

### Ausgabegroessen

gpt-image-2 liefert nur 1024x1024, 1024x1536 und 1536x1024. Alles oben ist auf 1024x1024 ausgelegt.

| Datei | Herkunft | Nachbearbeitung |
|---|---|---|
| `gebirge3.png` | Master | Naht retuschieren |
| `ter_ridge_snow.png` | Firn | Naht retuschieren, auf 512x512 |
| `obj_rockspire_1..3.png` | Objektblatt-Quadranten | freistellen, zentrieren |
| `ter_scree_cone.png` | Objektblatt unten rechts | freistellen, auf 512x384 |

## 12.4 Wasser

**Wasserkachel** - `ter_water.png` (512x512)
```
A seamless top-down GROUND texture of calm water for a stylized
cartoon medieval settlement-building game.

SURFACE: still deep water seen from directly above, in a DEEP
DESATURATED blue-green that sits harmoniously beside muted olive-green
meadow grass - never bright turquoise, never tropical, never
saturated. Broad soft darker patches suggest depth beneath the
surface, and a few sparse pale highlights hint at gentle wave crests.
Very low contrast overall, soft painterly transitions, no crisp
caustic network.

No foam, no spray, no shoreline, no reeds, no ripple rings, no
reflections of anything, no water plants - the shore edge is added by
the game.

This texture is laid over a 3D terrain mesh, so it must carry NO
lighting direction of its own: completely flat, even, ambient
illumination, no directional light, no cast shadows, no ambient
occlusion. All shading comes from the mesh.
...
## 12.5 Gebaeude-Effekte

**Sheet, vier Effekte** - `fx_steam.png, fx_flour.png, fx_sawdust.png, fx_rings.png` (192x256 / 256x256 / 256x256 / 256x128)
```
Stylized cartoon visual effect assets for a medieval settlement-
building game: soft painterly shapes with fully soft edges, no hard
contours, no black outlines, minimal fine detail, muted palette.

Generate this as ONE single image containing all four effects as
separate items, arranged in a row with generous empty space between
them so each can be cropped out individually afterward. All four share
the same soft painting technique and the same level of simplification.

Include exactly these four effects, side by side:

1. STEAM: a soft billowing plume of warm white steam rising and
widening upward, densest at the bottom where it emerges and fading
gradually to fully transparent at the top and along its edges. Tall
and narrow overall.

2. FLOUR DUST: a fine pale white cloud of drifting flour dust, a soft
diffuse smear spreading sideways and slightly downward, thinner and
wispier than the steam, with a few faint lighter specks caught in it,
fading to fully transparent at every edge.

3. SAWDUST: a light warm yellow cloud of sawdust puffing outward,
softer in the middle and speckled at its outer edges with a scatter of
small individual pale wood shavings and chips flying free, fading to
fully transparent at the edges.

4. WATER RINGS: three concentric flat rings spreading on a water
surface, seen at a slight oblique angle so they read as flattened
ovals rather than perfect circles. Pale blue-white, the innermost ring
brightest and the outermost faintest, each ring a soft thin band that
fades to fully transparent. No splash, no droplets, no water surface
behind them.

Fully transparent background everywhere. No panel borders, no grid
lines, no labels, no text, no numbers, no watermark, no UI, no
characters, no buildings, no scenery, no cast shadows.
```

## 12.6 Optional

**Sheet, Zusatz-Effekte** - `fx_embers.png, fx_smoke_dark.png` (128x256 bzw. wie fx_smoke)
```
Stylized cartoon visual effect assets for a medieval settlement-
building game: soft painterly shapes with fully soft edges, no hard
contours, no black outlines, minimal fine detail.

Generate this as ONE single image containing both effects as separate
items, side by side with generous empty space between them so each can
be cropped out individually afterward.

Include exactly these two effects:

1. EMBERS: a narrow rising column of small glowing forge sparks - a
scatter of tiny warm orange and yellow embers drifting upward, densest
and brightest near the bottom, thinning out and dimming toward the
top, each ember surrounded by a soft warm glow. Tall and narrow. No
flames, no smoke, no fire.

2. DARK SMOKE: a soft billowing plume of dark grey-brown smoke rising
and widening upward, noticeably darker and heavier than ordinary
chimney smoke, densest at the bottom and fading gradually to fully
transparent at the top and along its edges.

Fully transparent background everywhere. No panel borders, no grid
lines, no labels, no text, no numbers, no watermark, no UI, no
characters, no buildings, no scenery.
```

**Fluegelkreuz orthografisch** - `muehlenrad_ortho.png` (1024x1024)
```
A stylized cartoon windmill sail cross for a medieval settlement-
building game, matching a warm hand-painted earthy art style. The sail
cross ONLY - no tower, no building, no roof, no landscape.
...
degrees from a central hub, each blade a slim wooden lattice frame
with pale cream sailcloth stretched across it. At the very center sits
a chunky round wooden hub with an iron collar.

CRITICAL - perfect rotational symmetry: all four blades must be
exactly IDENTICAL in length, width, shape and detail, arranged at
exactly 90 degree intervals, so the cross looks the same after any
quarter turn. Seen straight on from the front, perfectly flat to the
camera, with NO tilt, NO foreshortening, NO perspective and NO sense
of depth - the tilt is applied by the game afterwards.

The hub must sit at the EXACT center of the image, with equal empty
margin on all four sides, generous enough that the blade tips stay
inside the frame at every rotation angle.

Lighting: soft and even across the whole cross, with gentle shading on
the wooden frames. The sailcloth must stay a soft muted cream with NO
blown-out white and no bright glare anywhere.

Fully transparent background. No text, no watermark, no UI, no tower,
no building, no sky, no ground, no cast shadow.
```

**Fisch-Ware** - `good_fish.png` (80x80)
```
A single stylized cartoon game icon of a fish for a medieval
settlement-building game: chunky simplified shape, minimal fine
surface detail, smooth matte surface, muted silvery blue-grey back
fading to a pale cream belly, a soft forked tail and a small simple
eye. No scales, no black outline.

CRITICAL - framing: the ENTIRE fish must fit inside the frame from
snout to the very tip of the tail fin, with clear empty margin on all
four sides. Nothing may touch or be cut off by any edge, especially
not the tail fin.

Seen in side view, lying horizontally, centered. Soft light from the
upper left. Fully transparent background. No text, no watermark, no
UI, no water, no plate, no other objects, no cast shadow.
```

**Sheet, sieben Werkzeugkoepfe** - `tool_axt, tool_schaufel, tool_klueppel, tool_hacke, tool_hammer, tool_angel, tool_saatsack` (je 128x128)
```
Stylized cartoon 3D tool assets for a medieval settlement-building
game: chunky simplified shapes with rounded edges, minimal fine
surface detail, smooth matte surfaces, warm muted earthy palette, no
black outlines.

Generate this as ONE single image containing all seven tools as
separate items, arranged in a row with generous empty space between
them so each can be cropped out individually afterward. All seven are
drawn at the same scale, in the same side view, with the same wood
tone and the same metal tone.

Every tool is shown in SIDE VIEW with its head at the upper left and
its handle running down toward the lower right at a consistent angle,
so the seven read as one matching set.

Include exactly these seven tools, side by side:

1. An axe with a broad iron blade and a wooden handle. 2. A shovel
with an iron-edged blade and a wooden handle. 3. A wooden mallet with
a heavy cylindrical head and a short handle. 4. A hoe with a flat iron
blade set at a right angle to a wooden handle. 5. A blacksmith's
hammer with a squared iron head and a wooden handle. 6. A fishing rod:
a slim tapering wooden pole with a line and a small hook hanging from
its tip. 7. A seed bag: a small closed cloth sack with a shoulder
strap, tied at the neck, with a few grains spilling at its foot.

Soft light from the upper left on every tool, brightening the upper-
left surfaces. NO cast shadow, NO drop shadow beneath any tool. Fully
transparent background. No text, no labels, no numbers, no watermark,
no UI, no background scenery, no hands, no characters.
```

**Sheet, drei Fahnenmenue-Knoepfe** - `ui_btn_weg.png, ui_btn_geologe.png, ui_btn_spaeher.png` (je 128x128, Referenz: `ui_ok.png`)
```
REFERENCE: use the attached image ui_ok. Match its exact button shape,
diameter, dark stained oak tone, raised gold rim, doming, shadow edge
and lighting precisely. Only the symbol on the face differs.

Generate this as ONE single image containing three such buttons side
by side with generous empty space between them so each can be cropped
out individually afterward. All three are identical in size, wood
tone, rim and lighting - only the emblem differs. Each emblem is one
bold, simple, clearly readable shape in warm ivory-gold, filling most
of the button face.

Include exactly these three buttons:

1. A signpost: a simple pointed board mounted on a short post,
pointing to the right. 2. A pickaxe seen in side view, head upper
left, handle lower right. 3. A spyglass: a short tapering telescope
tube seen from the side, with two visible ring bands.

Chunky simplified shapes, bold and readable: in the game these buttons
are drawn only about 48 pixels across, so no thin lines, no fine
detail, no small ornament.

Seen straight on from the front, flat to the camera, no perspective
distortion, no tilt. Soft light from the upper left. Fully transparent
background around and between the buttons. No text, no lettering, no
numbers, no watermark, no cast shadow on the background.
```
...
# 13. Grafikliste Nr. 9 (Bloecke A bis F)

Alle 48 Prompts vollstaendig ausgeschrieben. Zwei Dinge gelten durchgaengig und stehen deshalb in jedem Prompt drin statt nur einmal vorne:

**Flaechenkacheln tragen KEIN Licht.** Kein Richtungslicht, keine Schatten, keine Vignette, keine helle oder dunkle Ecke. Das Spiel multipliziert seine eigene Schattierung aus dem Hoehenmodell darauf; ein zweites eingebackenes Licht macht den Fels fleckig.

**Sprites tragen Licht von oben links** plus weichen Kontaktschatten direkt am Fuss, aber keinen langen Schlagschatten und keine Bodenplatte.

Einzige Ausnahme sind die Wandtexturen in Block B: dort gehoert das Relief ins Bild, die Beleuchtung bleibt aber schwerkraftbezogen - dunkel nur unter Vorspruengen, kein Licht von links oder rechts.

Bei Serien immer ein Sammelbild und danach zerschneiden, damit Pinselfuehrung und Palette ueber die Serie identisch bleiben. Ausnahme: nahtlos kachelbare Texturen muessen einzeln entstehen, sonst geht die Kachelbarkeit verloren.

Tipp fuer Block A: die erste gelungene Felskachel als Referenzbild an die drei folgenden Prompts haengen, sonst driftet die Koernung auseinander.

## 13.1 Block A - Flaechenkacheln (Standard)

**Felskachel 1 - ruhig** - `ter_fels_1.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Grobkoerniger Granit, warmgrau entsaettigt. Variante "ruhig": grosse
zusammenhaengende Flaeche, feines gleichmaessiges Korn, nur wenige
feine Risse. Sichtbare Schichtung, dunkle Fugenlinien.

VOLLKOMMEN GLEICHMAESSIGE DIFFUSE AUSLEUCHTUNG, kein Richtungslicht,
keine Schatten, keine Vignette, keine helle oder dunkle Ecke, keine
Perspektive, kein Rand. Kontrastumfang moderat - die Kachel darf die
Schattierung des Spiels nicht ueberlagern.

Palette: #BFB49B, #A2957C, #8A7E68, #6A5C4A, #57503F.

Keine Vegetation, keine losen Steine, keine Objekte, keine harten
schwarzen Outlines, kein Fotorealismus, kein 3D-Render-Look mit
Glanzlichtern, kein Text, kein Wasserzeichen. Handgemalter Cartoon-
Look, weiche Kanten. Seamless tileable in x und y.
```

**Felskachel 2 - gebaendert** - `ter_fels_2.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Grobkoerniger Granit, warmgrau entsaettigt. Variante "gebaendert":
ausgepraegte Schichtbaender, die klar und durchgehend durchs Bild
laufen. Sichtbare Schichtung, dunkle Fugenlinien.
...
**Felskachel 3 - zerklueftet** - `ter_fels_3.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Grobkoerniger Granit, warmgrau entsaettigt. Variante "zerklueftet":
dichtes Netz aus Rissen, viele kleine Bruchflaechen unterschiedlicher
Groesse. Sichtbare Schichtung, dunkle Fugenlinien.
...
**Felskachel 4 - grossplattig** - `ter_fels_4.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Grobkoerniger Granit, warmgrau entsaettigt. Variante "grossplattig":
wenige sehr grosse Platten, dafuer tiefe breite Fugen dazwischen.
Sichtbare Schichtung, dunkle Fugenlinien.
...
**Geroellkachel** - `ter_geroell.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Geroellflaeche, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Loser kantiger Schutt in drei Groessenklassen, dicht gepackt,
dazwischen feiner Grus. Keine runden Kiesel, keine Erde, kein Bewuchs.
...
## 13.1 Block A - Flaechenkacheln (Wueste)

**Felskachel 1 - ruhig** - `ter_fels_1_wueste.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Sandstein, waermer und sandiger, weichere Kanten, ausgepraegte
waagerechte Schichtung. Variante "ruhig": grosse zusammenhaengende
Flaeche, feines gleichmaessiges Korn, nur wenige feine Risse.
Sichtbare Schichtung, dunkle Fugenlinien.
...
Palette: #C8A575, #A07E52, #6E5334, #4A3722.
...
**Felskachel 2 - gebaendert** - `ter_fels_2_wueste.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Sandstein, waermer und sandiger, weichere Kanten, ausgepraegte
waagerechte Schichtung. Variante "gebaendert": ausgepraegte
Schichtbaender, die klar und durchgehend durchs Bild laufen. Sichtbare
Schichtung, dunkle Fugenlinien.
...
**Felskachel 3 - zerklueftet** - `ter_fels_3_wueste.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Sandstein, waermer und sandiger, weichere Kanten, ausgepraegte
waagerechte Schichtung. Variante "zerklueftet": dichtes Netz aus
Rissen, viele kleine Bruchflaechen unterschiedlicher Groesse.
Sichtbare Schichtung, dunkle Fugenlinien.
...
**Felskachel 4 - grossplattig** - `ter_fels_4_wueste.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Sandstein, waermer und sandiger, weichere Kanten, ausgepraegte
waagerechte Schichtung. Variante "grossplattig": wenige sehr grosse
Platten, dafuer tiefe breite Fugen dazwischen. Sichtbare Schichtung,
dunkle Fugenlinien.
...
**Geroellkachel** - `ter_geroell_wueste.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Geroellflaeche, Draufsicht senkrecht von oben,
1024x1024, handgemalt.
...
Loser kantiger Schutt in drei Groessenklassen, dicht gepackt,
dazwischen feiner Grus. Keine runden Kiesel, keine Erde, kein Bewuchs.
Der Schutt ist sandsteinfarben und etwas weicher gebrochen.
...
## 13.1 Block A - Flaechenkacheln (Winter)

**Felskachel 1 - ruhig** - `ter_fels_1_winter.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

kuehler Frostfels, kuehl entsaettigt, haerter und splittriger, feine
Frostrisse und Abplatzungen, Schneereste in den Fugen. Variante
"ruhig": grosse zusammenhaengende Flaeche, feines gleichmaessiges
Korn, nur wenige feine Risse. Sichtbare Schichtung, dunkle
Fugenlinien.
...
Palette: #C6CBD0, #A3ABB4, #7E8791, #5E6772.
...
**Felskachel 2 - gebaendert** - `ter_fels_2_winter.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.
...
kuehler Frostfels, kuehl entsaettigt, haerter und splittriger, feine
Frostrisse und Abplatzungen, Schneereste in den Fugen. Variante
"gebaendert": ausgepraegte Schichtbaender, die klar und durchgehend
durchs Bild laufen. Sichtbare Schichtung, dunkle Fugenlinien.
...
**Felskachel 3 - zerklueftet** - `ter_fels_3_winter.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.
...
kuehler Frostfels, kuehl entsaettigt, haerter und splittriger, feine
Frostrisse und Abplatzungen, Schneereste in den Fugen. Variante
"zerklueftet": dichtes Netz aus Rissen, viele kleine Bruchflaechen
unterschiedlicher Groesse. Sichtbare Schichtung, dunkle Fugenlinien.
...
**Felskachel 4 - grossplattig** - `ter_fels_4_winter.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.
...
kuehler Frostfels, kuehl entsaettigt, haerter und splittriger, feine
Frostrisse und Abplatzungen, Schneereste in den Fugen. Variante
"grossplattig": wenige sehr grosse Platten, dafuer tiefe breite Fugen
dazwischen. Sichtbare Schichtung, dunkle Fugenlinien.
...
**Geroellkachel** - `ter_geroell_winter.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Geroellflaeche, Draufsicht senkrecht von oben,
1024x1024, handgemalt.
...
Loser kantiger Schutt in drei Groessenklassen, dicht gepackt,
dazwischen feiner Grus. Keine runden Kiesel, keine Erde, kein Bewuchs.
Zwischen dem Schutt liegen Schneereste in den Zwischenraeumen.
...
## 13.1 Block A - Flaechenkacheln (Vulkan)

**Felskachel 1 - ruhig** - `ter_fels_1_vulkan.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Basalt, dunkel, blasig-poroes, teils saeulige Absonderung, gluehende
Risse. Variante "ruhig": grosse zusammenhaengende Flaeche, feines
gleichmaessiges Korn, nur wenige feine Risse. Sichtbare Schichtung,
dunkle Fugenlinien.
...
Palette: #6B5A55, #47393A, #2B2224 plus Glut #C4441E.
...
**Felskachel 2 - gebaendert** - `ter_fels_2_vulkan.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Basalt, dunkel, blasig-poroes, teils saeulige Absonderung, gluehende
Risse. Variante "gebaendert": ausgepraegte Schichtbaender, die klar
und durchgehend durchs Bild laufen. Sichtbare Schichtung, dunkle
Fugenlinien.
...
**Felskachel 3 - zerklueftet** - `ter_fels_3_vulkan.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Basalt, dunkel, blasig-poroes, teils saeulige Absonderung, gluehende
Risse. Variante "zerklueftet": dichtes Netz aus Rissen, viele kleine
Bruchflaechen unterschiedlicher Groesse. Sichtbare Schichtung, dunkle
Fugenlinien.
...
**Felskachel 4 - grossplattig** - `ter_fels_4_vulkan.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Gesteinstextur, Draufsicht senkrecht von oben,
1024x1024, handgemalt.

Basalt, dunkel, blasig-poroes, teils saeulige Absonderung, gluehende
Risse. Variante "grossplattig": wenige sehr grosse Platten, dafuer
tiefe breite Fugen dazwischen. Sichtbare Schichtung, dunkle
Fugenlinien.
...
**Geroellkachel** - `ter_geroell_vulkan.png` (1024x1024, nahtlos)
```
Nahtlos kachelbare Geroellflaeche, Draufsicht senkrecht von oben,
1024x1024, handgemalt.
...
Loser kantiger Schutt in drei Groessenklassen, dicht gepackt,
dazwischen feiner Grus. Keine runden Kiesel, keine Erde, kein Bewuchs.
Der Schutt ist dunkler Basalt, poroes, mit vereinzelt gluehenden
Spalten.
...
## 13.2 Block B - Wandtexturen

**Felswand** - `ter_rock_cliff.png` (1024x1536, nahtlos)
```
Nahtlos kachelbare Felswand, Seitenansicht, 1024x1536, handgemalt.

Massives geschichtetes Gestein: waagerechte Schichtfugen
unterschiedlicher Dicke, dazwischen kurze senkrechte Kluefte. Bloecke
kantig und abgeschlagen, klar verschieden gross, manche vorspringend,
manche zurueckgesetzt.

ENTSCHEIDEND: KEINE senkrechte Kluft laeuft ueber mehr als ein Drittel
der Bildhoehe. Jede endet an einer waagerechten Schichtfuge, versetzt
sich seitlich und laeuft erst dann weiter. Keine durchgehenden
senkrechten Linien ueber die volle Hoehe.

Schatten NUR unter Vorspruengen, schwerkraftbezogen. Kein Licht von
links oder rechts, keine Schlagschatten von aussen, keine Vignette.

Palette: #BFB49B, #A2957C, #8A7E68, #6A5C4A, #57503F.

Handgemalter Cartoon-Look, weiche Kanten, keine harten schwarzen
Outlines, kein Fotorealismus, kein Text. Seamless tileable in x und y.
```

**Felswand Variante** - `ter_rock_cliff2.png` (1024x1536, nahtlos)
```
Nahtlos kachelbare Felswand, Seitenansicht, 1024x1536, handgemalt.

Massives geschichtetes Gestein, Variante gegen den Wiederholungstakt:
die Schichtbaender laufen unregelmaessiger, sind teils leicht
verkippt, einzelne Baender laufen seitlich aus. Waagerechte
Schichtfugen unterschiedlicher Dicke, dazwischen kurze senkrechte
Kluefte. Bloecke kantig und abgeschlagen, klar verschieden gross,
manche vorspringend, manche zurueckgesetzt.
...
## 13.3 Block C - Felsobjekte

**Sheet, zehn Objekte** - `obj_findling_gross, obj_findling_mittel, obj_steingruppe, obj_blockstapel, obj_crag_3, obj_crag_4, obj_steinplatte, obj_krueppelbaum, deco_moosfleck, deco_grasbuschel_fels` (je 256x256, die drei grossen 512x512)
```
Sprite-Serie in EINEM Bild als Raster, isometrische Draufsicht mit
leichter Schraege, transparenter Hintergrund. Alle Objekte in gleicher
Palette und gleicher Pinselfuehrung, damit sie wie aus einem
Steinbruch wirken. Grosszuegiger Abstand zwischen den Objekten, damit
sie einzeln ausgeschnitten werden koennen.

WICHTIG: jedes Objekt sitzt BREIT IM BODEN. Kein Objekt steht auf
einem schmalen Fuss, keines hat einen Ueberhang, der seinen eigenen
Fuss verdeckt - aus der Vogelperspektive wuerde es sonst schweben.

Enthalte genau diese zehn Objekte:

1) Findling gross, kantig, geschichtet, zu einem Drittel im Boden
steckend 2) Findling mittel, gerundeter, Moosansatz an einer Seite 3)
Steingruppe klein: drei Brocken, einer davon halb vergraben 4)
Blockstapel: breite halb eingegrabene Basisplatte, darueber zwei bis
drei kleiner werdende Bloecke, KEIN Ueberhang, KEIN schmaler Fuss 5)
Felssporn: laeuft schraeg aus dem Hang, vorn hoch und kantig, nach
hinten flach in den Boden auslaufend 6) Felssporn gegenlaeufig
geneigt, gedrungener 7) umgestuerzte Steinplatte, schraeg auf zwei
Auflagern, mit Splitt am Fuss 8) abgestorbener Krueppelbaum, kahl,
windschief, wenige dicke Aeste 9) Moos- und Flechtenfleck als flaches
Decal, unregelmaessiger Rand, halbtransparent auslaufend 10)
Grasbueschel fuer den Uebergang am Felsfuss

Palette warmgrau: #BFB49B, #A2957C, #8A7E68, #6A5C4A, #57503F. Moos
gedaempftes Gruen, Krueppelbaum graubraunes Totholz.

Licht von OBEN LINKS bei allen Objekten gleich, weicher
Kontaktschatten direkt am Fuss, KEIN langer Schlagschatten, KEINE
Bodenplatte, KEIN Bodenteller. Freigestellt auf vollstaendig
transparentem Hintergrund.

Handgemalter Cartoon-Look, weiche Kanten, keine harten schwarzen
Outlines, kein Fotorealismus, kein 3D-Render-Look mit Glanzlichtern,
kein Text, kein Wasserzeichen, keine Menschen.
```

**Sheet, zwei Haufen** - `obj_steinhaufen.png, obj_kieshaufen.png` (je 384x288)
```
Zwei Sprites in EINEM Bild nebeneinander, isometrische Draufsicht mit
leichter Schraege, transparenter Hintergrund, grosszuegiger Abstand
zum Ausschneiden.

1) Steinhaufen: grobe kantige Bloecke, locker aufgeschuettet, breite
Basis, nach oben schmaler werdend 2) Kieshaufen: feiner Splitt,
flacher Kegel, an den Raendern duenn auslaufend statt hart abbrechend

Palette warmgrau: #BFB49B, #A2957C, #8A7E68, #6A5C4A, #57503F.
...
## 13.4 Block D - Bergbau

**Mineneingang Kohle** - `bld_coalmine.png` (512x512)
```
Isometrisches Spiel-Sprite, 512x512, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.

Mineneingang: schwerer Holzrahmen aus dunklen Balken ueber einem
tiefschwarzen Tunnelloch, in eine Felswand eingelassen, sodass der
untere Bildrand im Gestein verschwindet. Davor ein kleiner
Aushubhaufen mit schwarzen glaenzenden Brocken und eine grobe
Karrenspur. Seitlich am Rahmen ein kleines Holzschild mit dem Symbol
fuer Kohle.

Kraeftige, sofort lesbare Silhouette.

Licht von oben links, weicher Kontaktschatten direkt unter dem Objekt,
kein langer Schlagschatten, keine Bodenplatte. Keine harten schwarzen
Outlines, kein Fotorealismus, kein Text, kein Wasserzeichen.
```

**Mineneingang Eisenerz** - `bld_ironmine.png` (512x512)
```
Isometrisches Spiel-Sprite, 512x512, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.
...
tiefschwarzen Tunnelloch, in eine Felswand eingelassen, sodass der
untere Bildrand im Gestein verschwindet. Davor ein kleiner
Aushubhaufen mit rostroten Brocken und eine grobe Karrenspur. Seitlich
am Rahmen ein kleines Holzschild mit dem Symbol fuer Eisenerz.
...
**Mineneingang Gold** - `bld_goldmine.png` (512x512)
```
Isometrisches Spiel-Sprite, 512x512, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.
...
tiefschwarzen Tunnelloch, in eine Felswand eingelassen, sodass der
untere Bildrand im Gestein verschwindet. Davor ein kleiner
Aushubhaufen mit goldglaenzenden Koernern und eine grobe Karrenspur.
Seitlich am Rahmen ein kleines Holzschild mit dem Symbol fuer Gold.
...
**Mineneingang Stein** - `bld_granitemine.png` (512x512)
```
Isometrisches Spiel-Sprite, 512x512, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.
...
tiefschwarzen Tunnelloch, in eine Felswand eingelassen, sodass der
untere Bildrand im Gestein verschwindet. Davor ein kleiner
Aushubhaufen mit grauen Quadern und Werkzeug und eine grobe
Karrenspur. Seitlich am Rahmen ein kleines Holzschild mit dem Symbol
fuer Stein.
...
**Mine erschoepft Kohle** - `bld_coalmine_leer.png` (512x512)
```
Isometrisches Spiel-Sprite, 512x512, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.

Erschoepfter Mineneingang: derselbe schwere Holzrahmen, aber das
Tunnelloch ist mit schraeg vernagelten Brettern verschlossen. Das
Holzschild fuer Kohle haengt verwittert und schief. Der Aushubhaufen
mit schwarzen Brocken ist vergraut und mit Gras ueberwachsen, die
Karrenspur verwaschen.
...
**Mine erschoepft Eisenerz** - `bld_ironmine_leer.png` (512x512)
```
Isometrisches Spiel-Sprite, 512x512, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.
...
Erschoepfter Mineneingang: derselbe schwere Holzrahmen, aber das
Tunnelloch ist mit schraeg vernagelten Brettern verschlossen. Das
Holzschild fuer Eisenerz haengt verwittert und schief. Der
Aushubhaufen mit rostroten Brocken ist vergraut und mit Gras
ueberwachsen, die Karrenspur verwaschen.
...
**Mine erschoepft Gold** - `bld_goldmine_leer.png` (512x512)
```
Isometrisches Spiel-Sprite, 512x512, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.
...
Erschoepfter Mineneingang: derselbe schwere Holzrahmen, aber das
Tunnelloch ist mit schraeg vernagelten Brettern verschlossen. Das
Holzschild fuer Gold haengt verwittert und schief. Der Aushubhaufen
mit goldenen Koernern ist vergraut und mit Gras ueberwachsen, die
Karrenspur verwaschen.
...
**Mine erschoepft Stein** - `bld_granitemine_leer.png` (512x512)
```
Isometrisches Spiel-Sprite, 512x512, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.
...
Erschoepfter Mineneingang: derselbe schwere Holzrahmen, aber das
Tunnelloch ist mit schraeg vernagelten Brettern verschlossen. Das
Holzschild fuer Stein haengt verwittert und schief. Der Aushubhaufen
mit grauen Quadern ist vergraut und mit Gras ueberwachsen, die
Karrenspur verwaschen.
...
**Aushubhaufen** - `deco_aushub.png` (256x128)
```
Isometrisches Spiel-Sprite, 256x128, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.

Kleiner Schutthaufen aus grobem Abraum, breite flache Basis,
unregelmaessiger Umriss, an den Raendern in einzelne Brocken
auslaufend.

Palette warmgrau: #A2957C, #8A7E68, #6A5C4A, #57503F.

Licht von oben links, weicher Kontaktschatten direkt am Fuss, kein
langer Schlagschatten, keine Bodenplatte. Keine harten schwarzen
Outlines, kein Text.
```

**Karrenspur** - `deco_karrenspur.png` (256x128)
```
Flaches Boden-Decal, 256x128, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.

Zwei ausgefahrene Radspuren im Grus, parallel verlaufend, leicht
unregelmaessig, mit aufgeworfenem Feinmaterial an den Raendern. An
beiden Enden weich in vollstaendige Transparenz auslaufend, damit sich
mehrere Spuren aneinanderreihen lassen.

SEHR FLACH: das Decal liegt auf dem Boden, es hat KEIN Volumen, KEINE
Hoehe, KEINEN Kontaktschatten und wirft keinen Schatten.

Palette warmgrau: #A2957C, #8A7E68, #6A5C4A.

Keine harten schwarzen Outlines, kein Text.
```

**Sheet, Steinvorkommen vier Abbaustufen** - `obj_stein_1.png bis _4.png` (je 256x256)
```
Isometrische Sprite-Serie in EINEM Bild, vier Objekte nebeneinander,
transparenter Hintergrund, grosszuegiger Abstand zum Ausschneiden,
handgemalter Cartoon-Look.

Dasselbe Steinvorkommen in vier Abbaustufen:

1) massiger Felsblock-Haufen in voller Groesse 2) etwa zwei Drittel
davon, mit frischen hellen Bruchkanten und Meisselspuren 3) etwa ein
Drittel, deutlich flacher, Splitt drumherum 4) nur noch Reststumpf und
Steinsplitter am Boden

ENTSCHEIDEND: die Grundflaeche bleibt ueber alle vier Stufen an
derselben Stelle und in derselben Breite - die Stufen werden im Spiel
gegeneinander ausgetauscht und duerfen dabei nicht springen. Nur die
Hoehe und die Masse nehmen ab.
...
**Sheet, vier Geologenschilder** - `sign_coal, sign_iron, sign_gold, sign_granite` (je 128x128)
```
Isometrische Sprite-Serie in EINEM Bild, vier Schilder nebeneinander,
je 128x128 Zuschnitt, transparenter Hintergrund, grosszuegiger Abstand
zum Ausschneiden, handgemalter Cartoon-Look.

Kleines Holzschild auf zwei in den Boden gerammten Pfloecken, grob
gezimmert, Holz verwittert. In der Mitte ein rundes Symbolfeld mit dem
Ressourcenzeichen. Alle vier Schilder identisch in Form, Groesse und
Machart - nur das Symbol unterscheidet sich:

1) Kohle: schwarze glaenzende Brocken 2) Eisenerz: rostrote Brocken 3)
Gold: goldglaenzende Koerner 4) Stein: graue Quader
...
## 13.5 Block E - Brand und Ruine

**Sheet, Brand klein** - `fx_burn_s_1 bis _3 + bld_ruin_s` (Mass wie das jeweilige Gebaeudebild)
```
Isometrische Sprite-Serie in EINEM Bild, vier Zustaende nebeneinander,
transparenter Hintergrund, handgemalter Cartoon-Look, grosszuegiger
Abstand zum Ausschneiden.

Ein kleines Holzhaus mit Strohdach in vier Zustaenden, alle mit
GLEICHER Grundflaeche, gleicher Groesse und gleichem Blickwinkel,
damit sie im Spiel gegeneinander ausgetauscht werden koennen:

1) Feuer im Dachstuhl: erste Flammen schlagen unter der Dachdeckung
hervor, duenne graue Rauchfahne, das Gebaeude noch weitgehend intakt
2) lodernder Vollbrand: Dach eingestuerzt, kraeftige orange Flammen
schlagen hoch, dichter dunkler Rauch 3) rauchende Truemmer: keine
offenen Flammen mehr, nur Glut in den Resten, verkohlte Balken,
aufsteigender Restrauch 4) kalte RUINE: verkohlte Balkenstuempfe,
eingestuerztes Mauerwerk, Asche und Schutt. KEINE Flammen, KEIN Rauch,
KEINE Glut.

Flammen warm und lesbar gemalt, kein Realismus-Feuer, keine
Partikeleffekte, keine Funken.

Licht von oben links, weicher Kontaktschatten direkt am Fuss, kein
langer Schlagschatten, keine Bodenplatte. Keine harten schwarzen
Outlines, kein Fotorealismus, kein Text.
```

**Sheet, Brand mittel** - `fx_burn_m_1 bis _3 + bld_ruin_m` (Mass wie das jeweilige Gebaeudebild)
```
Isometrische Sprite-Serie in EINEM Bild, vier Zustaende nebeneinander,
transparenter Hintergrund, handgemalter Cartoon-Look, grosszuegiger
Abstand zum Ausschneiden.

Ein mittelgrosses Fachwerkhaus mit Schindeldach in vier Zustaenden,
alle mit GLEICHER Grundflaeche, gleicher Groesse und gleichem
Blickwinkel, damit sie im Spiel gegeneinander ausgetauscht werden
koennen:
...
**Sheet, Brand gross** - `fx_burn_l_1 bis _3 + bld_ruin_l` (Mass wie das jeweilige Gebaeudebild)
```
Isometrische Sprite-Serie in EINEM Bild, vier Zustaende nebeneinander,
transparenter Hintergrund, handgemalter Cartoon-Look, grosszuegiger
Abstand zum Ausschneiden.

Eine grosse Scheune mit hohem Dach in vier Zustaenden, alle mit
GLEICHER Grundflaeche, gleicher Groesse und gleichem Blickwinkel,
damit sie im Spiel gegeneinander ausgetauscht werden koennen:
...
**Sheet, Brand Turm** - `fx_burn_turm_1 bis _3 + bld_ruin_turm` (Mass wie das jeweilige Gebaeudebild)
```
Isometrische Sprite-Serie in EINEM Bild, vier Zustaende nebeneinander,
transparenter Hintergrund, handgemalter Cartoon-Look, grosszuegiger
Abstand zum Ausschneiden.

Ein steinerner Wehrturm mit Holzdach in vier Zustaenden, alle mit
GLEICHER Grundflaeche, gleicher Groesse und gleichem Blickwinkel,
damit sie im Spiel gegeneinander ausgetauscht werden koennen:
...
**Sheet, Stolleneinsturz** - `fx_burn_mine_1 bis _3 + bld_ruin_mine` (Mass wie das Minenbild)
```
Isometrische Sprite-Serie in EINEM Bild, vier Zustaende nebeneinander,
transparenter Hintergrund, handgemalter Cartoon-Look, grosszuegiger
Abstand zum Ausschneiden.

Einstuerzender Mineneingang in vier Zustaenden, alle mit GLEICHER
Grundflaeche und gleichem Blickwinkel:

1) der Holzrahmen des Stollens splittert, erste Balken brechen, feiner
Staub rieselt herab 2) der Rahmen bricht zusammen, Gestein rutscht von
oben nach, dichte graubraune Staubwolke quillt aus dem Loch 3) der
Eingang ist weitgehend verschuettet, nur noch Restaub in der Luft,
gebrochene Balkenenden ragen aus dem Geroell 4) kalte RUINE: der
Stollen vollstaendig verschuettet, ein Haufen Geroell mit ein paar
gesplitterten Balkenenden, kein Staub, keine Bewegung

Kein Feuer, keine Flammen - eine Mine stuerzt ein, sie brennt nicht.
...
## 13.6 Block F - Kleinkram

**Statusschild fehlt etwas** - `ui_status_warn.png` (64x64)
```
Kleines gemaltes Statusschild als Spiel-Sprite, 64x64, freigestellt
auf transparentem Hintergrund, handgemalter Cartoon-Look.

Ein kleines grob gezimmertes Holzschild mit einem grossen, kraeftigen
Ausrufezeichen. Das Schild fuellt fast das ganze Bild, das Symbol ist
gross, kraeftig und auch bei 64 Pixeln sofort lesbar - keine duennen
Linien, keine feinen Details.

Licht von oben links, dezenter Kontaktschatten. Kein langer
Schlagschatten, keine Bodenplatte, keine harten schwarzen Outlines,
kein Text, keine Buchstaben, keine Zahlen.
```

**Statusschild steht still** - `ui_status_sleep.png` (64x64)
```
Kleines gemaltes Statusschild als Spiel-Sprite, 64x64, freigestellt
auf transparentem Hintergrund, handgemalter Cartoon-Look.

Ein kleines grob gezimmertes Holzschild mit einem grossen, kraeftigen
Schlaf-Symbol aus drei aufsteigenden Zzz-Zeichen. Das Schild fuellt
fast das ganze Bild, das Symbol ist gross, kraeftig und auch bei 64
Pixeln sofort lesbar - keine duennen Linien, keine feinen Details.
...
**Statusschild Werkzeug fehlt** - `ui_status_tool.png` (64x64)
```
Kleines gemaltes Statusschild als Spiel-Sprite, 64x64, freigestellt
auf transparentem Hintergrund, handgemalter Cartoon-Look.

Ein kleines grob gezimmertes Holzschild mit einem grossen, kraeftigen
Symbol aus gekreuztem Hammer und Zange. Das Schild fuellt fast das
ganze Bild, das Symbol ist gross, kraeftig und auch bei 64 Pixeln
sofort lesbar - keine duennen Linien, keine feinen Details.
...
**Wasserschaum** - `fx_schaum.png` (256x64)
```
Wasserschaum-Saum als Spiel-Textur, 256x64, freigestellt auf
transparentem Hintergrund, handgemalter Cartoon-Look.

Ein waagerechtes Band aus weichem weissem Wasserschaum mit kleinen
Blasen, unten dichter, nach oben in vollstaendige Transparenz
auslaufend. Der Rand ist unregelmaessig und organisch, nie eine gerade
Linie.

NAHTLOS IN X: der linke und der rechte Bildrand passen ohne sichtbare
Naht aneinander, damit sich der Saum entlang einer Uferlinie
fortsetzen laesst.

Keine Spritzer, keine Tropfen, keine Wasserflaeche dahinter, keine
harten schwarzen Outlines, kein Text.
```

**Wasserglanz, vier Phasen** - `fx_glanz.png` (256x64)
```
Animationsblatt als Spiel-Textur, 256x64, freigestellt auf
transparentem Hintergrund, handgemalter Cartoon-Look.

VIER gleich grosse Zellen nebeneinander, je 64x64, jede mit weichen
Lichtreflexen auf einer Wasseroberflaeche: kurze helle Striche und
kleine ovale Glanzflecken in Weiss und hellem Blauweiss. Von Zelle zu
Zelle wandern und veraendern sich die Reflexe leicht, sodass die vier
Zellen nacheinander abgespielt eine ruhige Kraeuselbewegung ergeben.

Alles ausser den Reflexen ist vollstaendig transparent - keine
Wasserflaeche, kein Hintergrund, keine Zellrahmen, keine Trennlinien,
kein Text.
```

**Grenzpfosten auf Fels** - `obj_borderpost_fels.png` (128x192)
```
Isometrisches Spiel-Sprite, 128x192, freigestellt auf transparentem
Hintergrund, handgemalter Cartoon-Look.

Grenzpfosten fuer felsigen Untergrund: ein schlanker weisser Holzpfahl
mit einem farbigen Band unter der Spitze, der NICHT in der Erde
steckt, sondern in einem Haufen locker aufgeschichteter Steine am Fuss
verkeilt ist. Die Steine bilden eine breite, stabile Basis, kantig und
warmgrau.

Das farbige Band ist eine flache, gleichmaessige Farbflaeche ohne
Muster und ohne Schattierung, damit es im Spiel auf die Spielerfarbe
eingefaerbt werden kann.

Palette Steine: #BFB49B, #A2957C, #8A7E68, #6A5C4A.

Licht von oben links, weicher Kontaktschatten direkt am Fuss, kein
langer Schlagschatten, keine Bodenplatte, kein Erdloch. Keine harten
schwarzen Outlines, kein Text.
```
